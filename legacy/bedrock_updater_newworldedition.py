...
class BedrockUpdaterApp:
    ...
    def log(self, message: str, level: str = "info"):
        """Central logging helper for the UI and the log file."""
    ...
    def parse_server_properties(self, filepath: Path) -> Dict[str, str]:
        """Parse server.properties into a dictionary, automatically handling directory paths."""
        props = {}
        
        # SIKKERHEDS-CHECK: Hvis stien er en mappe, så find filen indeni
        if filepath.is_dir():
        ...
        try:
            ...
            # Nu logger vi kun fejlen, hvis det ikke er en "mappe-fejl"
            self.log(f"Error parsing properties: {e}", "error")
            
        return props

    def save_server_properties(self, filepath: Path, props: Dict[str, str]):
        """Save properties back to file while preserving formatting."""
        if not filepath.exists():
    ...
    def setup_ui(self):
        ...
        # 3. ADD THE NEW PROPERTIES TAB HERE (The Transplant)
        # We put it here so 'self.notebook' already exists!
        self.properties_editor = ServerPropertiesEditor(self.notebook, self)
        self.notebook.add(self.properties_editor, text="📝 Properties") 
        # Note: I used a memo icon 📝 because Settings already uses the Gear ⚙️
    ...
    def create_menu(self):
        ...
        tools_menu.add_command(label="Check for Server Updates", command=self.check_for_updates)
        tools_menu.add_command(label="Download Latest Server", command=self.open_download_page)
        ...
        help_menu.add_command(label="Bedrock Server Documentation", 
                             command=lambda: open_url("https://minecraft.wiki/w/Bedrock_Dedicated_Server"))
    ...
    def setup_main_tab(self):
        ...
        ttk.Label(header, text="Update Bedrock Server", 
        ...
        # Canvas with scrollbar for many options
    ...
    def setup_server_tab(self):
        ...
        for cmd in ["list", "save hold", "save query", "save resume", "say Hello!"]:
            ttk.Button(quick_frame, text=cmd, 
    ...
    def setup_backup_tab(self):
        ...
        ttk.Button(control_frame, text="💾 Create Backup Now", 
                  command=self.manual_backup).pack(side=tk.LEFT, padx=5)
        ttk.Button(control_frame, text="🧹 Cleanup Old Backups", 
                  command=self.cleanup_backups).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(action_frame, text="🔄 Restore Selected", 
        ...
        ttk.Button(action_frame, text="📂 Open in Explorer", 
                  command=self.open_selected_backup).pack(side=tk.LEFT, padx=5)
    ...
    def setup_worlds_tab(self):
        ...
        ttk.Button(control_frame, text="📂 Open Worlds Folder", 
                  command=self.open_worlds_folder).pack(side=tk.LEFT, padx=5)
        ...
        # --- NY SEKTION: Active World Management (Row 3) ---
        ...
        ttk.Button(entry_frame, text="Set Active / Create", 
                   command=self.quick_save_level_name).pack(side=tk.RIGHT)
    ...
    def setup_settings_tab(self):
        ...
        max_backup_spin = ttk.Spinbox(backup_frame, from_=1, to=50, width=10, textvariable=self.max_backups_var)
        max_backup_spin.grid(row=0, column=1, sticky="w", padx=10)
        
        self.auto_cleanup_var = tk.BooleanVar(value=self.config.get("auto_cleanup_backups", True))
        ttk.Checkbutton(backup_frame, text="Automatically cleanup old backups after update",
        ...
        ttk.Checkbutton(backup_frame, text="Compress backups (ZIP format, slower but smaller)",
        ...
        ttk.Checkbutton(update_frame, text="Automatically stop server before update",
        ...
        ttk.Checkbutton(update_frame, text="Automatically start server after update",
        ...
        ttk.Checkbutton(update_frame, text="Check for server updates on application start",
        ...
        font_spin = ttk.Spinbox(ui_frame, from_=6, to=24, width=10, textvariable=self.font_size_var)
        font_spin.grid(row=0, column=1, sticky="w", padx=10)
        ...
        ttk.Label(about_frame, text="A comprehensive tool for managing Minecraft Bedrock Dedicated Servers.",
    ...
    def validate_inputs(self, event=None):
        ...
        if server_path:
            ...
            if is_valid:
                ...
                # Special path: Folder exists but is EMPTY
                self.server_status.config(text="❓ Folder is empty (Ready for Install)", foreground="blue")
                valid = False # Keep button disabled for now
                
                # Check if a ZIP is already selected to offer installation
        ...
        # Update the main button state (only for normal updates)
        self.update_button.config(state=tk.NORMAL if valid and not self.is_updating else tk.DISABLED)
        # Refresh the properties editor if it exists
        if hasattr(self, 'properties_editor'):
            self.properties_editor.load_properties()
    ...
    def update_server_info(self):
        ...
        info_lines = []
        info_lines.append(f"Server Name: {props.get('server-name', 'Unknown')}")
        info_lines.append(f"Game Mode: {props.get('gamemode', 'Unknown')} | Difficulty: {props.get('difficulty', 'Unknown')}")
        info_lines.append(f"Max Players: {props.get('max-players', 'Unknown')} | Port: {props.get('server-port', '19132')}")
        info_lines.append(f"Worlds: {len(worlds)} | Total size: {format_size(get_folder_size(server_path / 'worlds'))}")
    ...
    def start_update(self):
        ...
        # --- NEW: Check if it's a first-time install ---
        # We assume it's a new install if 'server.properties' doesn't exist yet
        ...
        if is_first_install:
            if not messagebox.askyesno("First-Time Install", 
                ...
                return
            # Skip backup/preservation logic for fresh installs
            self.is_updating = True
            self.update_button.config(state=tk.DISABLED)
            threading.Thread(target=self.perform_update, daemon=True).start()
            return

        # --- EXISTING SAFETY LOGIC (For actual updates) ---
        preserve = self.get_preserve_list()
    ...
    def perform_update(self):
        ...
        try:
            ...
            if not is_fresh:
                ...
                # --- STEP 2: CLEAR SERVER (Skip if fresh) ---
            ...
            # Cleanup old backups (Skip if fresh)
            ...
            # Re-scan the folder so the UI turns GREEN
            ...
            self.is_updating = False
            self.is_first_install = False # Reset installer flag
    ...
    def on_world_select(self, event):
        ...
        if selected:
            ...
            world_name = item["values"][0]
            self.world_info_label.config(text=f"Selected: {world_name}")
    ...
    def open_download_page(self):
        """Handles the 'Download Latest' button click."""
        # Check if we have a manually entered URL from the Wiki flow
        if hasattr(self, 'last_manual_url') and self.last_manual_url:
            ...
            # CALL THE CORRECT FUNCTION: download_updates
    ...
    def download_updates(self, url: str):
        """
        Initiates the download of the server ZIP from the provided URL.
        """
        if not url:
            ...
            try:
                ...
                with urllib.request.urlopen(req) as response, open(target_path, 'wb') as out_file:
                    ...
                    while True:
                        ...
                        if not buffer:
                            break
                        out_file.write(buffer) # Write data
    ...
    def manual_version_input(self):
        """Opens wiki and asks for version number manually."""
        ...
        if v:
            ...
            # This is where we store it for the download button
    ...
    def validate_server_files(self):
        ...
        essential = [SERVER_EXECUTABLE, "server.properties"]
        for f in essential:
        ...
        if sys.platform != "win32":
            ...
            if exe.exists() and not os.access(exe, os.X_OK):
                issues.append(f"{SERVER_EXECUTABLE} is not executable")
...
Features:
• Safe server updates with automatic backup
• Server control with live console
...
class ServerPropertiesEditor(ttk.Frame):
    ...
    def load_properties(self):
        ...
        # Vi bruger en manuel tæller (row_index), fordi vi springer level-name over
        row_index = 0
        for key, value in props.items():
            # FILTER: Hvis det er level-name, så spring den over her
            # (Den bliver nu håndteret i Worlds-fanen i stedet)
